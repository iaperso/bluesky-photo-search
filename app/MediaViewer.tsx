...
function sourceForCard(card:HTMLElement){const src=card.dataset.mediaSrc||'';const kind=card.dataset.mediaKind==='video'?'video':'image';if(!src)return null;return{kind:kind as MediaKind,src,poster:card.dataset.mediaPoster||null}}
...
export default function MediaViewer(){
 ...
 const openAt=useCallback((index:number)=>{const cards=mediaCards();const source=cards[index]?sourceForCard(cards[index]):null;if(!source)return;setState({open:true,index,kind:source.kind,src:source.src,poster:source.poster})},[])
 ...
 return <div className={`mediaViewer ${state.open?'isOpen':''}`} role="dialog" aria-modal={state.open?'true':'false'} aria-label="Média plein écran" onTouchStart={e=>{if(!state.open)return;const t=e.touches[0];touchStart.current={x:t.clientX,y:t.clientY}} onTouchEnd={e=>{if(!state.open)return;const s=touchStart.current;touchStart.current=null;if(!s)return;const t=e.changedTouches[0];const dx=t.clientX-s.x;const dy=t.clientY-s.y;if(Math.max(Math.abs(dx),Math.abs(dy))<45)return;Math.abs(dy)>=Math.abs(dx)?move(dy<0?1:-1):move(dx<0?1:-1)}}>
  ...
  {state.open&&<><button className="mediaViewerClose" type="button" aria-label="Fermer" onClick={close}>×</button><button className="mediaViewerPrev" type="button" aria-label="Média précédent" onClick={()=>move(-1)}>‹</button><button className="mediaViewerNext" type="button" aria-label="Média suivant" onClick={()=>move(1)}>›</button></>}
 </div>
}
