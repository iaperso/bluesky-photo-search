...
export default function MediaViewer(){
 ...
 return <div className={`mediaViewer ${state.open?'isOpen':''}`} role="dialog" aria-modal={state.open?'true':'false'} aria-label="Média plein écran" onTouchStart={e=>{if(!state.open)return;const target=e.target as HTMLElement;if(target.closest('button')||e.touches.length!==1){touchStart.current=null;return}const t=e.touches[0];touchStart.current={x:t.clientX,y:t.clientY}}} onTouchEnd={e=>{if(!state.open)return;const target=e.target as HTMLElement;if(target.closest('button')){touchStart.current=null;return}const s=touchStart.current;touchStart.current=null;if(!s)return;if((window.visualViewport?.scale??1)>1.01)return;const t=e.changedTouches[0];const dx=t.clientX-s.x;const dy=t.clientY-s.y;if(Math.max(Math.abs(dx),Math.abs(dy))<45)return;Math.abs(dy)>=Math.abs(dx)?move(dy<0?1:-1):move(dx<0?1:-1)}} onTouchCancel={()=>{touchStart.current=null}}>
