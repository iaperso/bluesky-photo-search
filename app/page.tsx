...
function VideoCard({video,mediaKey,author}:{video:Video;mediaKey:string;author:string}){
 ...
 useEffect(()=>{
  ...
  if(el.canPlayType('application/vnd.apple.mpegurl')){if(el.src!==video.playlist)el.src=video.playlist;el.preload='auto';el.load()}
   ...
   hlsRef.current=hls
   hls.on(Hls.Events.ERROR,(_event,data)=>{if(!data.fatal)return;if(data.type===Hls.ErrorTypes.NETWORK_ERROR){hls.startLoad();return}if(data.type===Hls.ErrorTypes.MEDIA_ERROR){hls.recoverMediaError();return}setFailed(true);setReady(false)})
   hls.loadSource(video.playlist);hls.attachMedia(el)
  }
 ...
 async function startPlayback(){if(!started){setStarted(true);requestAnimationFrame(()=>videoRef.current?.play().catch(()=>{}));return}if(failed){setFailed(false);setReady(false);videoRef.current?.play().catch(()=>{})}}
...
export default function Home(){
 ...
 const discoveryMissesRef=useRef(0)
 const discoveryBatchRef=useRef(0)
 const accountsQuery=useMemo(()=>accounts.map(a=>`@${a}`).join(','),[accounts])
 const seedQuery=useMemo(()=>accounts.join(','),[accounts])

 useEffect(()=>{try{setAdultConfirmed(localStorage.getItem(AGE_KEY)==='yes');const saved=JSON.parse(localStorage.getItem(ACCOUNTS_KEY)??'[]');if(Array.isArray(saved))setAccounts(saved.filter(x=>typeof x==='string').map(cleanAccount).filter(Boolean).slice(0,MAX_ACCOUNTS))}catch{setAdultConfirmed(false)}finally{setAgeChecked(true)}},[])
 useEffect(()=>{const onChanged=(event:Event)=>{const detail=(event as CustomEvent).detail;if(Array.isArray(detail))setAccounts(detail.filter(x=>typeof x==='string').map(cleanAccount).filter(Boolean).slice(0,MAX_ACCOUNTS))};window.addEventListener('visual-search-accounts-changed',onChanged);return()=>window.removeEventListener('visual-search-accounts-changed',onChanged)},[])
 function saveAccounts(next:string[]){const normalized=[...new Set(next.map(cleanAccount).filter(Boolean))].slice(0,MAX_ACCOUNTS);setAccounts(normalized);try{localStorage.setItem(ACCOUNTS_KEY,JSON.stringify(normalized))}catch{}}
 ...
 async function fetchDiscovery(nextMode:Mode){
  ...
  const params=new URLSearchParams({kind:nextMode==='videos'?'video':'image',batch:String(++discoveryBatchRef.current)})
 ...
 async function fetchFeed(nextQuery:string,nextCursor?:string|null,append=false,nextMode:Mode=mode,discoveryOnly=false){
  ...
  if(nextMode==='accounts'&&!cleaned)return
  if(!cleaned&&nextMode!=='accounts'&&!seedQuery&&nextMode!=='videos')return
  if(append&&nextCursor&&lastCursorRef.current===nextCursor)return
  loadingRef.current=true
  if(append&&nextCursor)lastCursorRef.current=nextCursor
  setLoading(true);setFailed(false)
  try{
   let data:SearchResponse
   if(discoveryOnly||(!cleaned&&nextMode!=='accounts')) data=await fetchDiscovery(nextMode)
   else{
    const endpoint=nextMode==='accounts'?'/api/account-feed':nextMode==='videos'?'/api/videos':'/api/search'
    const params=new URLSearchParams({q:cleaned})
    if(nextMode==='search'){params.set('sort','latest');params.set('mode','adult')}
    if(nextCursor)params.set('cursor',nextCursor)
    const response=await fetch(`${endpoint}?${params}`,{cache:'no-store'})
    const primary:SearchResponse=await response.json()
    if(!response.ok)throw new Error(primary.error||'search')
   ...
   const newPosts=append?freshPosts(posts,data.posts):uniquePosts(data.posts)
   const combined=uniquePosts(append?[...posts,...newPosts]:newPosts)
   setPosts(combined)
   if(discoveryOnly&&append)discoveryMissesRef.current=newPosts.length?0:discoveryMissesRef.current+1
   else if(!append)discoveryMissesRef.current=0
   ...
   if(nextMode==='videos'){
    ...
    setDisplayMedia(append?[...displayMedia,...fresh]:mediaItems(newPosts))
   ...
   setCursor(stalled?null:data.cursor)
   if(!stalled)lastCursorRef.current=null
   setActiveQuery(cleaned||DISCOVERY_QUERY)
  }catch{
   setFailed(true);lastCursorRef.current=null
   if(!append){setPosts([]);setDisplayImages([]);setDisplayVideos([]);setDisplayMedia([]);setCursor(null)}
 ...
 useEffect(()=>{
  ...
  const observer=new IntersectionObserver(entries=>{
   ...
   if(activeQuery===DISCOVERY_QUERY){
    if(discoveryMissesRef.current>=2)return
    void fetchFeed('',null,true,mode,true)
    return
   }
   if(!cursor)return
   const q=mode==='accounts'?accountsQuery:activeQuery
   if(q)void fetchFeed(q,cursor,true,mode)
  },{rootMargin:'1400px 0px 1400px 0px',threshold:0})
  observer.observe(target)
  return()=>observer.disconnect()
 },[cursor,activeQuery,mode,accountsQuery,posts.length,displayVideos.length,displayImages.length,displayMedia.length])

 function submit(e:FormEvent){
  e.preventDefault();lastCursorRef.current=null;discoveryMissesRef.current=0
  if(mode==='accounts'){
   const account=cleanAccount(query)
   if(!account||accounts.includes(account)||accounts.length>=MAX_ACCOUNTS)return
   const next=[...accounts,account]
   saveAccounts(next);setQuery('')
   void fetchFeed(next.map(x=>`@${x}`).join(','),null,false,'accounts')
   return
  }
  if(query.trim())void fetchFeed(query)
 }

 function switchMode(nextMode:Mode){
  if(nextMode===mode||loadingRef.current)return
  lastCursorRef.current=null;discoveryMissesRef.current=0
  setMode(nextMode);setPosts([]);setDisplayImages([]);setDisplayVideos([]);setDisplayMedia([]);setCursor(null);setActiveQuery('');setFailed(false);setQuery('')
  if(nextMode==='accounts'&&accounts.length)setTimeout(()=>void fetchFeed(accounts.map(x=>`@${x}`).join(','),null,false,'accounts'),0)
  else if(nextMode==='videos'||(nextMode!=='accounts'&&accounts.length))setTimeout(()=>void fetchFeed('',null,false,nextMode,true),0)
 }

 function removeAccount(account:string){
  const next=accounts.filter(x=>x!==account)
  saveAccounts(next);lastCursorRef.current=null;discoveryMissesRef.current=0
  setPosts([]);setDisplayImages([]);setDisplayVideos([]);setDisplayMedia([]);setCursor(null);setActiveQuery('')
  if(mode==='accounts'&&next.length)setTimeout(()=>void fetchFeed(next.map(x=>`@${x}`).join(','),null,false,'accounts'),0)
  else if(mode==='videos'||(mode!=='accounts'&&next.length))setTimeout(()=>void fetchFeed('',null,false,mode,true),0)
 }
 ...
 const showSentinel=hasResults&&Boolean(activeQuery)&&(activeQuery===DISCOVERY_QUERY||Boolean(cursor))&&!(activeQuery===DISCOVERY_QUERY&&discoveryMissesRef.current>=2)
 ...
 return <main className="adultMode">
  ...
  {hasResults&&<section className="resultsSection" aria-label="Résultats">
   ...
   {showSentinel&&<div className={`infiniteSentinel ${loading?'loading':''}`} ref={infiniteSentinel} aria-hidden="true"><span/></div>}
  </section>}
 </main>
}
