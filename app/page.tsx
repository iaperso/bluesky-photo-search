...
function VideoCard({video,mediaKey}:{video:Video;mediaKey:string}){
 ...
 return <div className={`videoCard ${orientation} ${started?'videoStarted':''} ${failed?'videoFailed':''}`} key={mediaKey} style={{aspectRatio:ratio}} data-video-src={video.playlist} data-video-poster={video.thumbnail??''}>
